...
export default function Navbar() {
  ...
  const getLinkClass = (href: string) => {
    ...
    return `text-sm font-medium transition-colors ${active ? "text-white underline underline-offset-8 decoration-blue-400" : "text-gray-300 hover:text-white"}`;
  };
    ...
    <nav className="bg-black/90 backdrop-blur-md border-b border-gray-800 px-4 py-4 fixed top-0 w-full z-40">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          ...
          <h1 className="text-2xl font-bold text-white hidden sm:block">Stream Zone</h1>
        </Link>
        ...
        {isLoggedIn && perfilActual && (
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 bg-gray-800/60 px-4 py-2 rounded-full border border-gray-700">
              ...
              <span className="text-gray-200 text-sm font-medium">{perfilActual.nombre}</span>
            </div>
            <button
